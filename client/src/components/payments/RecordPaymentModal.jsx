 ...
 * Dismissal is standardized on backdrop click + Escape, with a dirty-form
 * confirmation gate so unsaved edits are never silently lost (#55).
 */
...
const RecordPaymentModal = ({ isOpen, onClose, onSubmit, receipt, loading = false }) => {
  ...
  const t = fr.modals.recordPayment;
  ...
  const paymentMethods = [
    ...
    { value: 'check', label: t.methods.check },
  ...
  const validateForm = () => {
    ...
    if (!formData.payment_date) {
      newErrors.payment_date = t.errDateRequired;
    } else {
      ...
      if (paymentDate > today) {
        newErrors.payment_date = t.errDateFuture;
      }
    }

    // Validate payment method
    if (!formData.payment_method) {
      newErrors.payment_method = t.errMethodRequired;
    }
